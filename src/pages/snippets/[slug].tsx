...
const SnippetPage: FC<Props> = ({
  meta: { title, description, date, keywords, tags, authors },
  source,
}) => {
  return (
    <Page title={title} description={description} keywords={keywords}>
      <Container className="max-w-6xl py-8 md:py-16 space-y-16">
...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  return {
    paths: slugs.map((slug) => `/snippets/${slug}`),
    fallback: false,
  };
};
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  const { slug } = params;
  const snippet = await getSnippetFromSlug(slug as string);
